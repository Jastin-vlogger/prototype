...
const OneFile = () => {
  ...
  const fetchProducts = useCallback(async (page = 1) => {
    ...
    try {
      const resp = await fetch(`/api/products?page=${page}&limit=20`);
      const data = await resp.json();
      const mapped = (data.products || []).map(p => ({
        id: p.PRODUCT_ID,
        name: p["Product Name"],
        price: p["Sale Price"],
        originalPrice: (p["Sale Price"] * 1.2).toFixed(2),
        category: p["Category"],
        mainCategory: p["Department"],
        subCategory: p["Group"],
        rating: 4.5,
        reviews: 10,
        images: ["https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&q=80&w=800"],
        description: p["Remarks"] || "High quality product from our catalog."
      }));
      setProducts(mapped);
    ...
    <div className={`sticky top-32 p-8 rounded-[2rem] border ${cardClass} ${isDarkMode ? 'border-white/10' : 'border-gray-200'}`}>
      ...
      <div className="space-y-4 mb-8">
        <div className={`flex justify-between text-sm ${textMutedClass}`}>
          ...
          <span>AED {totals?.subtotal?.toFixed(2) || '0.00'}</span>
        </div>
        ...
        <div className={`flex justify-between text-sm ${textMutedClass}`}>
          <span>Tax (5%)</span>
          <span>AED {totals?.tax?.toFixed(2) || '0.00'}</span>
        </div>
        <div className={`flex justify-between text-sm ${textMutedClass}`}>
          <span>Shipping</span>
        ...
        <div className={`flex justify-between text-xl font-bold ${textHeadingClass}`}>
          <span>Total</span>
          <span>AED {totals?.total?.toFixed(2) || (items.reduce((sum, i) => sum + (getItemDetails(i.name).price * i.qty), 0) * 1.05).toFixed(2)}</span>
        </div>
      </div>
      {buttonText && (
        <button
              ...
              <div className="space-y-12">
                ...
                <div className="flex justify-center items-center gap-4 py-8">
                  <button
                    ...
                    className={`p-3 rounded-xl border transition-all ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-500 hover:text-white'} ${isDarkMode ? 'border-white/10' : 'border-gray-200'}`}
                  >
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  <span className={`font-bold ${textHeadingClass}`}>Page {currentPage} of {totalPages}</span>
                  <button
                    disabled={currentPage === totalPages}
                    onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))}
                    className={`p-3 rounded-xl border transition-all ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-500 hover:text-white'} ${isDarkMode ? 'border-white/10' : 'border-gray-200'}`}
                  >
                    <ArrowRight className="w-5 h-5" />
                  </button>
                </div>
  ...
  const renderServiceBookingFlow = () => {
    ...
    const StepIndicator = ({ step, title }) => (
      <div className="flex items-center">
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${serviceBookingStep === step ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' :
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="hidden sm:flex items-center gap-3">
              <div className="flex flex-col items-end">
                <span className={`text-xs font-medium opacity-60 ${textMutedClass}`}>
                  {currentUser ? 'Welcome back,' : 'Guest User'}
                </span>
              ...
              <button
                onClick={() => currentUser ? (window.location.href = '/customerdash') : (window.location.href = '/customerdash')}
                className={`p-2.5 rounded-xl transition-all shadow-md group ${isDarkMode ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-white text-indigo-600 hover:bg-indigo-50 border border-indigo-100'}`}
              >
