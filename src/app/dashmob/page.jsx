...
const ToastContainer = ({ toasts, removeToast }) => (
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map(toast => (
            <div key={toast.id} className={`pointer-events-auto transform transition-all duration-300 animate-in slide-in-from-right fade-in flex items-center p-4 rounded-xl shadow-lg border w-80 backdrop-blur-md ${
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={`flex flex-col items-center justify-center p-3 rounded-lg border text-sm font-medium transition-all ${
                            itemDetails[item.id].action === type.id
                                    ...
                                    <div className="flex justify-between mb-2 relative">
                                        ...
                                        {doc.steps.map((step, idx) => (
                                            <div key={idx} className="flex flex-col items-center relative z-10">
                                                <div className={`w-5 h-5 rounded-full flex items-center justify-center border-2 text-[8px] font-bold transition-colors ${
...
const LoginScreen = ({ setUserState, setCurrentView, setNeedsPasswordChange, language, setLanguage, isDark, toggleTheme }) => {
    ...
    const handleLogin = (simulatedType) => { if (simulatedType === 'new') { setUserState('onboarding'); } else if (simulatedType === 'existing') { setUserState('approved'); setCurrentView('dashboard'); } else if (simulatedType === 'pendingApproval') { setUserState('pendingApproval'); setCurrentView('dashboard'); } };
    const submitSignup = () => { 
    ...
    const renderView = () => {
        if (view === 'signup') {
             return (
                <div className="space-y-5 animate-in slide-in-from-right">
                    ...
                    <FormInput id="email" name="email" label="Business Email" type="email" icon={Mail} placeholder="you@company.com" className={inputClass} labelClassName={labelClass} value={signupData.email} onChange={handleInputChange} />
                    <div className="relative">
                        <FormInput id="mobile" name="mobile" label="Mobile Number" type="tel" icon={Phone} placeholder="+971 50 000 0000" className={inputClass} labelClassName={labelClass} value={signupData.mobile} onChange={handleInputChange} />
        ...
        return (<div className="space-y-6 animate-in slide-in-from-left"><div className="text-start mb-8"><h2 className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>Welcome Back</h2><p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} mt-2 text-sm`}>Please sign in to access your dashboard</p></div><FormSelect id="userType" label="I am a..." icon={User} value={userType} onChange={(e) => setUserType(e.target.value)} className={inputClass} labelClassName={labelClass}><option value="vendor">Vendor</option><option value="customer">Customer</option></FormSelect><FormInput id="username" name="username" label="Username or Email" type="text" icon={User} placeholder="customer@company.com" className={inputClass} labelClassName={labelClass} value={loginData.username} onChange={handleLoginChange} /><div><FormInput id="password" name="password" label="Password" type="password" icon={Lock} placeholder="••••••••" className={inputClass} labelClassName={labelClass} value={loginData.password} onChange={handleLoginChange} /><div className="text-end -mt-2"><button onClick={() => console.log("Password reset link sent to your email.")} className="text-sm font-semibold text-blue-600 hover:text-blue-800 transition-colors">Forgot password?</button></div></div><FormSelect id="language" label="Preferred Language" icon={Languages} value={language} onChange={(e) => setLanguage(e.target.value)} className={inputClass} labelClassName={labelClass}><option value="en">English</option><option value="ar">العربية (Arabic)</option></FormSelect><PrimaryButton onClick={() => handleLogin('existing')} className="bg-gradient-to-r from-red-600 to-blue-700 hover:from-red-700 hover:to-blue-800 shadow-xl shadow-blue-200 text-white font-bold py-3.5 text-base rounded-xl">Sign In</PrimaryButton><p className={`text-center text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>New user?{' '}<button onClick={() => setView('signup')} className="font-bold text-blue-600 hover:text-blue-800 underline decoration-blue-200 hover:decoration-blue-500">Create Account</button></p><div className="opacity-0 hover:opacity-100 transition-opacity absolute bottom-2 left-2 flex gap-1"><button onClick={() => handleLogin('new')} className="text-[10px] bg-red-100 p-1">Dev: New</button><button onClick={() => handleLogin('pendingApproval')} className="text-[10px] bg-orange-100 p-1">Dev: Pending</button></div></div>);
    };
      ...
      <div className="mt-6">
        ...
        {tab === 'ledger' && (
           <Card noPadding={true}>
              ...
              <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
                  {filteredLedger.length === 0 ? (
                      <div className="p-8 text-center text-slate-500 dark:text-slate-400">No records matching your filters.</div>
                  ) : (
                      filteredLedger.map((row) => (
                          <div key={row.id} className="p-4 bg-white dark:bg-slate-900 space-y-3">
                              <div className="flex justify-between items-start">
                                  <div>
                                      <p className="text-xs text-slate-400 dark:text-slate-500 font-medium mb-0.5">{row.date}</p>
                                      <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{row.desc}</p>
                                  </div>
                                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${row.type === 'inv' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : row.type === 'pay' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'}`}>
                                      {row.type === 'inv' ? 'Invoice' : row.type === 'pay' ? 'Payment' : 'Credit Note'}
                                  </span>
                              </div>
                              <div className="flex justify-between items-end border-t border-slate-50 dark:border-slate-800 pt-3">
                                  <div>
                                      ...
                                      <p className="text-sm font-medium text-slate-600 dark:text-slate-300">AED {row.bal.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                                  </div>
                                  <div className="text-right">
                                      <p className="text-[10px] font-bold text-slate-400 uppercase">Amount</p>
                                      <p className={`text-lg font-black ${row.dr > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                                          {row.dr > 0 ? `- AED ${row.dr.toFixed(2)}` : `+ AED ${row.cr.toFixed(2)}`}
                                      </p>
                                  </div>
                              </div>
                          </div>
        ...
        {tab === 'invoices' && (
          <Card noPadding={true} className="overflow-hidden flex flex-col">
            ...
            <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
                {filteredInvoices.length === 0 ? (
                    <div className="p-8 text-center text-slate-500 dark:text-slate-400">No invoices match your filters.</div>
                ) : (
                    filteredInvoices.map((inv) => (
                        <div key={inv.invoiceId} className={`p-4 transition-colors ${selectedOrderIds.includes(inv.orderId) ? 'bg-blue-50/40 dark:bg-blue-900/10' : 'bg-white dark:bg-slate-900'}`}>
                            <div className="flex items-start gap-3">
                                <div className="pt-1">
                                    <input 
                                        type="checkbox" 
                                        className="w-5 h-5 rounded border-gray-300 dark:border-slate-600 dark:bg-slate-700 text-blue-600 focus:ring-blue-500 disabled:opacity-30"
                                        checked={selectedOrderIds.includes(inv.orderId)}
                                        onChange={() => toggleInvoiceSelection(inv.orderId)}
                                        disabled={inv.status !== 'Pending Payment'}
                                    />
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start mb-1">
                                        <button onClick={() => setSelectedInvoice(inv)} className="text-sm font-bold text-blue-600 dark:text-blue-400 truncate hover:underline">
                                            {inv.invoiceId}
                                        </button>
                                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase whitespace-nowrap ${inv.status === 'Paid' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : inv.status === 'Pending Payment' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'}`}>
                                            {inv.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <p className="text-lg font-black text-slate-900 dark:text-white">AED {inv.amount.toFixed(2)}</p>
                                        <button onClick={() => handleDownload(`${inv.invoiceId}.pdf`)} className="p-2 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                            <Download className="w-4 h-4" />
                                        </button>
                                    </div>
                                </div>
          ...
          <Card noPadding={true}>
             ...
             <div className="hidden md:block overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    <thead className="bg-slate-50 dark:bg-slate-900/50">
                        <tr>
                            <th className="px-6 py-3 text-start text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Reference</th>
                            <th className="px-6 py-3 text-start text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Date</th>
                            <th className="px-6 py-3 text-start text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Description</th>
                            <th className="px-6 py-3 text-end text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount</th>
                            <th className="px-6 py-3 text-end text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-900">
                        {[
                          ...
                          { id: 'TXN-9960', date: '25 Nov 2025, 11:00 AM', desc: 'Paid Invoice #INV-2925', amount: -60.00, status: 'Success' },
                            ...
                            <tr key={txn.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <td className="px-6 py-4 text-sm font-medium text-slate-900 dark:text-white whitespace-nowrap">{txn.id}</td>
                                <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 whitespace-nowrap">{txn.date}</td>
                                <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">{txn.desc}</td>
                                <td className={`px-6 py-4 text-sm text-end font-bold whitespace-nowrap ${txn.amount > 0 ? 'text-green-600 dark:text-green-400' : 'text-slate-900 dark:text-white'}`}>
                                    {txn.amount > 0 ? '+' : ''} AED {Math.abs(txn.amount).toFixed(2)}
                                </td>
                                <td className="px-6 py-4 text-end whitespace-nowrap">
                                    <span className="text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-1 rounded-full font-bold uppercase">{txn.status}</span>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                  ...
                  { id: 'TXN-9960', date: '25 Nov 2025, 11:00 AM', desc: 'Paid Invoice #INV-2925', amount: -60.00, status: 'Success' },
                    ...
                    <div key={txn.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <div className="text-right">
                             ...
                             <span className="text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded uppercase font-bold">{txn.status}</span>
                        </div>
                    </div>
                ))}
             </div>
                                ...
                                <div className="flex justify-between mb-2">
                                    {doc.steps.map((step, idx) => (
                                        <div key={idx} className="flex flex-col items-center flex-1 relative">
                                            ...
                                            {/* Dot */}
                                            <div className={`w-5 h-5 rounded-full z-10 flex items-center justify-center border-2 text-[10px] font-bold transition-colors ${
                                                step.status === 'completed' ? 'bg-green-500 border-green-500 text-white' : 
                                                step.status === 'error' ? 'bg-red-500 border-red-500 text-white' :
...
// --- NEW MENU PAGE (Replaces Sidebar) ---
const MenuPage = ({ setCurrentView, onLogout }) => {
  const menuItems = [
    { id: 'returns', label: 'Returns & Refunds', icon: Undo2, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-900/20', desc: 'Track reports and issues' },
    { id: 'saved-items', label: 'Saved Items', icon: Heart, color: 'text-pink-600', bg: 'bg-pink-50 dark:bg-pink-900/20', desc: 'Your favorite products' },
    { id: 'kyc', label: 'KYC Documents', icon: FileText, color: 'text-cyan-600', bg: 'bg-cyan-50 dark:bg-cyan-900/20', desc: 'Manage compliance files' },
    { id: 'settings', label: 'Account Settings', icon: Settings, color: 'text-slate-600 dark:text-slate-300', bg: 'bg-slate-100 dark:bg-slate-800', desc: 'Profile and preferences' },
    { id: 'help', label: 'Support', icon: CircleHelp, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-900/20', desc: 'FAQs and contact info' },
  ];

  return (
    <div className="space-y-6 animate-in fade-in duration-300 max-w-lg mx-auto pb-20">
      <div className="flex flex-col items-center justify-center p-8 bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 text-center">
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold text-3xl shadow-inner mb-4">AC</div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Abreco Customer</h2>
        <p className="text-slate-500 dark:text-slate-400 mb-4">admin@abrecocustomer.com</p>
        <button onClick={() => setCurrentView('settings')} className="text-sm bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-4 py-2 rounded-full font-bold">Edit Profile</button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {menuItems.map(item => (
          <button 
            ...
            className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-all active:scale-95 text-left"
          >
      ...
      <button onClick={onLogout} className="w-full p-4 flex items-center justify-center gap-2 text-red-600 font-bold bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900/30 rounded-2xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shadow-sm">
        <LogOut className="w-5 h-5" /> Sign Out
      </button>
      
      <p className="text-center text-xs text-slate-400 py-4">App Version 2.5.0</p>
    </div>
  );
};

// --- UPDATED BOTTOM NAVIGATION (Replaces Sidebar & MobileBottomNav) ---
const BottomNavigation = ({ currentView, setCurrentView }) => {
  const navItems = [
    { id: 'dashboard', label: 'Home', icon: Home },
    { id: 'orders', label: 'Orders', icon: Package },
    { id: 'financials', label: 'Finance', icon: Banknote },
    { id: 'services', label: 'Services', icon: Wrench },
    { id: 'menu', label: 'Menu', icon: Menu }, // Menu Tab handles "More"
  ];

  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 pb-safe z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      <div className="flex justify-around items-center h-16 sm:h-20 max-w-screen-xl mx-auto">
        {navItems.map((item) => {
           // Highlight 'menu' tab if we are in any sub-page that isn't a main tab
           const isSubPage = ['settings', 'kyc', 'returns', 'saved-items', 'help'].includes(currentView);
           const isActive = currentView === item.id || (item.id === 'menu' && isSubPage);
           
           return (
             <button
               key={item.id}
               onClick={() => setCurrentView(item.id)}
               className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-all duration-200 active:scale-90 group relative ${isActive ? 'text-red-600 dark:text-red-500' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
             >
               <div className={`relative p-1.5 rounded-xl transition-all ${isActive ? 'bg-red-50 dark:bg-red-900/20 translate-y-[-2px]' : ''}`}>
                   <item.icon className={`w-6 h-6 sm:w-7 sm:h-7 ${isActive ? 'fill-current opacity-100' : 'opacity-80'}`} strokeWidth={isActive ? 2.5 : 2} />
               </div>
               <span className={`text-[10px] sm:text-xs font-bold tracking-tight transition-colors ${isActive ? 'text-red-600 dark:text-red-500' : 'text-slate-400 dark:text-slate-500'}`}>{item.label}</span>
             </button>
           );
        })}
      </div>
...
const Header = ({ onLogout, currentView, isDark, toggleTheme, cartCount, openCart }) => { 
    ...
    return (
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm px-4 py-3 flex justify-between items-center sticky top-0 z-40 transition-colors duration-300">
            <div className="flex items-center gap-3">
                <div className="p-2 bg-gradient-to-br from-red-600 to-blue-700 rounded-xl text-white shadow-md">
                    <Package2 className="w-6 h-6" />
                </div>
                <div>
                    <h1 className="text-lg font-black text-slate-900 dark:text-white capitalize leading-tight">
                        {currentView === 'kyc' ? 'Documents' : 
                         currentView === 'dashboard' ? 'Overview' : 
                         currentView === 'saved-items' ? 'Saved' :
                         currentView.replace('-', ' ')}
                    </h1>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden sm:block">Portal</span>
                </div>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 rtl:space-x-reverse">
                 {/* Visit Shop Button */}
                 <a 
                    href="https://ibillprototype.netlify.app/customer4"
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all border ${isDark ? 'bg-slate-800 border-slate-700 text-blue-400 hover:bg-slate-700 hover:text-blue-300' : 'bg-white border-slate-200 text-slate-700 hover:text-blue-600 hover:border-blue-200 hover:shadow-sm'}`}
                 >
                 ...
                 {/* Cart Button */}
                 <button onClick={openCart} className="p-2 relative rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                     <ShoppingBag className="w-6 h-6" />
                     {cartCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-600 rounded-full text-[10px] text-white flex items-center justify-center font-bold shadow-sm ring-2 ring-white dark:ring-slate-900">{cartCount}</span>}
                 </button>

                 {/* Notifications */}
                 <div className="relative" ref={notifRef}>
                    <button onClick={() => setIsNotifOpen(!isNotifOpen)} className="p-2 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"><Bell className="w-6 h-6" /></button>
                    {isNotifOpen && (
                        <div className="absolute top-full end-0 mt-3 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 z-50 p-2 animate-in fade-in zoom-in-95 origin-top-right">
                            <h4 className="text-sm font-bold p-3 text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-700 mb-1">Notifications</h4>
                            <div className="space-y-1 max-h-60 overflow-y-auto">
                                <div className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl cursor-pointer transition-colors">
                                    <div className="flex items-center gap-2 mb-1">
                                        <div className="w-2 h-2 rounded-full bg-blue-500"></div>
                                        <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Order Out for Delivery</p>
                                    </div>
                                    <p className="text-[10px] text-slate-500 dark:text-slate-400 ml-4">Order #ORD-2931 is on its way.</p>
                                </div>
                                <div className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl cursor-pointer transition-colors">
                                    <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <p className="text-[10px] text-slate-500 dark:text-slate-400 ml-4">Invoice #INV-2930 is pending.</p>
                                </div>
                            </div>
                        </div>
                    )}
...
export default function App() {
  ...
  const handleLogout = () => { setUserState('loggedOut'); setCurrentView('dashboard'); setLanguage('en'); setNeedsPasswordChange(false); };
    ...
    <div className={`${isDark ? 'dark' : ''} h-full w-full`}>
        {userState === 'loggedOut' ? (
             <div dir={language === 'ar' ? 'rtl' : 'ltr'}>
                <LoginScreen setUserState={setUserState} setCurrentView={setCurrentView} setNeedsPasswordChange={setNeedsPasswordChange} language={language} setLanguage={setLanguage} isDark={isDark} toggleTheme={toggleTheme} />
             </div>
            ...
            <div className={`flex h-screen w-full font-inter transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-gradient-to-r from-cyan-50 via-purple-50 to-pink-50'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
                ...
                {/* NO SIDEBAR - App View Layout */}
                
                <main className="flex-1 flex flex-col overflow-y-auto z-10 relative pb-24 no-scrollbar">
                    <Header 
                    ...
                    />
                    <div className="flex-grow p-4 sm:p-6 max-w-screen-xl mx-auto w-full">
                        {(() => {
                            switch (currentView) {
                                ...
                                case 'menu': return <MenuPage setCurrentView={setCurrentView} onLogout={handleLogout} />;
                ...
                {/* Permanent Mobile-Style Bottom Navigation */}
                <BottomNavigation 
                    currentView={currentView} 
