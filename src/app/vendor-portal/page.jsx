...
const UploadDocumentsScreen = ({ docStatus, setDocStatus, paymentTerm, setPaymentTerm, setUserState }) => {
  ...
  const handleAddCustomDoc = () => {
    ...
    setCustomDocs([...customDocs, { key, title: newDocName }]);
    setDocStatus(prev => ({ ...prev, [key]: 'missing' }));
    setNewDocName('');
  };

  const handleRemoveCustomDoc = (key) => {
    setCustomDocs(customDocs.filter(d => d.key !== key));
    setDocStatus(prev => {
        const newState = { ...prev };
        delete newState[key];
        return newState;
  ...
  const DocumentCard = ({ title, docKey, status, isCustom }) => (
    <div className="bg-white p-6 rounded-xl shadow-lg border-t border-slate-100 relative group">
      {isCustom && (
          <button 
            onClick={() => handleRemoveCustomDoc(docKey)}
    ...
    <div className="min-h-screen w-full bg-slate-100 p-4 md:p-8 font-sans antialiased text-slate-600 text-sm">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          ...
          {customDocs.map(doc => (
              <DocumentCard key={doc.key} title={doc.title} docKey={doc.key} status={docStatus[doc.key]} isCustom={true} />
          ))}
        </div>

        <div className="bg-slate-50 border border-dashed border-slate-300 p-6 rounded-xl mt-6 transition-all focus-within:border-red-300 focus-within:bg-slate-100">
            <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                <Plus className="w-4 h-4 text-slate-500" /> Add Other Documents
            </h3>
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1">
                    <input 
                        type="text" 
                        placeholder="Document Name (e.g. VAT Certificate, Liability Insurance)" 
                        className="block w-full px-4 py-2.5 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 sm:text-sm"
                        value={newDocName}
                ...
                <button 
                    ...
                    className="px-6 py-2.5 bg-slate-900 text-white rounded-lg font-bold text-sm hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap transition-colors"
                >
                    Add Field
                </button>
            </div>
...
const BiddingManagementPage = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {/* Navigation Tabs - ENHANCED MULTI-COLORED */}
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
                <div className="p-3">
                    <div className="flex overflow-x-auto hide-scrollbar gap-3 snap-x p-1">
                        {tabs.map((tab) => {
                            const isActive = activeTab === tab.id;
                            
                            // Dynamic Styles
                            const styles = {
                                slate:   isActive ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 bg-slate-100 hover:bg-slate-200',
                                blue:    isActive ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-slate-600 bg-blue-50 hover:bg-blue-100',
                                indigo:  isActive ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-600 bg-indigo-50 hover:bg-indigo-100',
                                emerald: isActive ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 bg-emerald-50 hover:bg-emerald-100',
                                rose:    isActive ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'text-slate-600 bg-rose-50 hover:bg-rose-100',
                            };
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold rounded-full transition-all duration-200 flex-shrink-0 whitespace-nowrap snap-start
                                        ${currentStyle}
                                    `}
                                >
                                    <tab.icon className={`w-4 h-4 ${isActive ? 'text-white' : 'opacity-70'}`} />
                                    <span>{tab.label}</span>
                                </button>
                            );
                        })}
                ...
                <Modal 
                    ...
                    footer={
                        <div className="flex justify-end gap-3 w-full">
                            <button onClick={() => setShowDetailModal(false)} className="px-4 py-3 border border-slate-300 rounded-xl text-slate-600 font-bold hover:bg-slate-50 active:scale-95 transition-all">Close</button>
                    ...
                    footer={
                        <div className="flex justify-between items-center w-full">
                            ...
                            <div className="flex gap-3 w-full sm:w-auto">
                                <button onClick={() => setShowSubmitModal(false)} className="flex-1 sm:flex-none px-4 py-3 border border-slate-300 rounded-xl text-slate-600 font-bold hover:bg-slate-50 active:scale-95 transition-all">Cancel</button>
                                <PrimaryButton onClick={handleSubmitBid} className="flex-1 sm:flex-none w-auto">{isEditing ? 'Update Bid' : 'Submit Bid'}</PrimaryButton>
                            </div>
                        </div>
                    }
                >
                    <div className="space-y-6">
                        <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="font-bold text-indigo-900 text-sm">{selectedBid.summary}</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div className="border border-slate-300 border-dashed rounded-xl px-4 py-2 flex items-center justify-between bg-slate-50 h-[42px]">
                                    <span className="text-sm text-slate-500">Upload Quotation / Spec PDF</span>
                                    <button className="text-xs bg-white border border-slate-300 px-2 py-1 rounded-lg shadow-sm hover:bg-slate-100">Browse</button>
                                </div>
                            </div>
                            <div className="md:col-span-2">
                                ...
                                <textarea 
                                    rows={3}
                                    className="block w-full p-3 border border-slate-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 sm:text-sm"
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
                ...
                <div className="border-b border-slate-200 p-3 bg-white">
                    <div className="flex overflow-x-auto hide-scrollbar gap-3 snap-x">
                        {tabs.map((tab) => {
                            ...
                            const styles = {
                                slate:   isActive ? 'bg-slate-900 text-white shadow-lg shadow-slate-200' : 'text-slate-600 bg-slate-100 hover:bg-slate-200',
                                amber:   isActive ? 'bg-amber-500 text-white shadow-lg shadow-amber-200' : 'text-slate-600 bg-amber-50 hover:bg-amber-100',
                                emerald: isActive ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200' : 'text-slate-600 bg-emerald-50 hover:bg-emerald-100',
                            };
                            const currentStyle = styles[tab.color] || styles.slate;
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`
                                        flex items-center justify-center gap-2 px-4 py-2 text-sm font-bold rounded-full transition-all duration-200 flex-shrink-0 whitespace-nowrap snap-start
                                        ${currentStyle}
                                    `}
                                >
                                    <div className="flex items-center gap-2">
                                        <tab.icon className={`w-4 h-4 ${isActive ? 'text-white' : 'opacity-70'} flex-shrink-0`} />
                                        <span>{tab.label}</span>
                                    </div>
                ...
                {/* 3. Dynamic Table */}
                <>
                    {/* Mobile App-Like Card List */}
                    <div className="lg:hidden space-y-4 p-4 bg-slate-50">
                        {filteredGRNs.length > 0 ? filteredGRNs.map((grn) => (
                            <div key={grn.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 active:scale-[0.99] transition-transform duration-150">
                                ...
                                <div className="text-xs text-slate-600 mb-3 grid grid-cols-2 gap-2">
                                    <div className="bg-slate-50 p-2 rounded-lg">
                                        <span className="block text-[10px] text-slate-400 uppercase">PO Number</span>
                                        <span className="font-mono font-medium">{grn.poNumber}</span>
                                    </div>
                                    <div className="bg-slate-50 p-2 rounded-lg">
                                        <span className="block text-[10px] text-slate-400 uppercase">Invoice</span>
                                        <span className="font-mono font-medium">{grn.invoiceNumber || 'N/A'}</span>
                                    </div>
                                </div>
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100 mb-4">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="font-bold text-slate-900">{grn.totalReceivedQty} Units</span>
                                    </div>
                                    {(grn.shortageQty > 0 || grn.damageQty > 0) ? (
                                        <div className="text-right">
                                            <span className="text-[10px] text-red-600 font-bold uppercase flex items-center gap-1"><AlertCircle className="w-3 h-3"/> Issues</span>
                                            <span className="text-xs font-bold text-red-600">S: {grn.shortageQty} | D: {grn.damageQty}</span>
                                        </div>
                                    ) : (
                                        <div className="text-right">
                                            <span className="text-[10px] text-green-600 font-bold uppercase flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Condition</span>
                                            <span className="text-xs font-bold text-green-600">Perfect</span>
                                        </div>
                                    )}
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => handleViewDetails(grn)} className="flex-1 py-2.5 border border-slate-300 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-center gap-2">
                                        <Eye className="w-4 h-4"/> View Details
                                    </button>
                                    {activeTab !== 'Pending' && (
                                        <button onClick={() => handleDownload(grn.grnNumber)} className="py-2.5 px-4 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200">
                ...
                <Modal 
                    ...
                    footer={
                        <div className="flex justify-between items-center w-full">
                            ...
                            <button onClick={() => setShowDetailModal(false)} className="px-4 py-3 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 text-sm font-bold w-full sm:w-auto">Close</button>
                        </div>
                    }
                >
                    <div className="space-y-6">
                                    ...
                                    <div key={idx} className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 text-center">
                                            <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                                <div className="text-[10px] uppercase text-slate-400 font-bold">Ordered</div>
                                                <div className="font-medium text-slate-700 text-sm">{item.orderedQty} <span className="text-[10px] font-normal">{item.unit}</span></div>
                                            </div>
                                            <div className="bg-blue-50 p-2 rounded-lg border border-blue-100">
                                                <div className="text-[10px] uppercase text-blue-400 font-bold">Received</div>
                                                <div className="font-bold text-blue-700 text-sm">{item.receivedQty}</div>
                                            </div>
                                            <div className="bg-green-50 p-2 rounded-lg border border-green-100">
                                                <div className="text-[10px] uppercase text-green-400 font-bold">Accepted</div>
                                                <div className="font-bold text-green-700 text-sm">{item.acceptedQty}</div>
                                            </div>
                                        </div>
                                        {(item.shortageQty > 0 || item.damageQty > 0) && (
                                            <div className="mt-3 flex gap-3 text-xs font-bold justify-end border-t border-slate-50 pt-2">
                                                {item.shortageQty > 0 && <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded">Short: {item.shortageQty}</span>}
                                                {item.damageQty > 0 && <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded">Dmg: {item.damageQty}</span>}
                                            </div>
                                        )}
                                    </div>
                                ))}
...
const DashboardView = ({ setCurrentView }) => {
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Unified Responsive Grid for Stats (Vertical on Mobile, Grid on Desktop) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <button onClick={() => setCurrentView('orders')} className="text-start transition-transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none">
          <StatCard title="New Orders" value="48" change="+12.5%" changeType="positive" icon={ShoppingCart} details="Last 30 days" variant="orange" />
        </button>
        <button onClick={() => setCurrentView('products-new')} className="text-start transition-transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none">
        ...
        <button onClick={() => setCurrentView('payments')} className="text-start transition-transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none">
                        ...
                        {tabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`
                                        relative flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold rounded-full transition-all duration-200 flex-shrink-0 whitespace-nowrap snap-start
                                        ${isActive 
                                            ? `${activeStyles[tab.color]}` 
                                            : 'text-slate-600 bg-slate-100 hover:bg-slate-200'
                                        }
                                    `}
                                >
                                    <Icon className={`w-4 h-4 ${isActive ? 'text-white' : 'text-current opacity-70'}`} />
                                    <span>{tab.label}</span>
                                    {count > 0 && (
                                        <span className={`
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
                <div className="border-b border-slate-200 p-3">
                    <div className="flex overflow-x-auto hide-scrollbar gap-3 snap-x">
                        {tabs.map((tab) => {
                            ...
                            const styles = {
                                slate:   isActive ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 bg-slate-100 hover:bg-slate-200',
                                amber:   isActive ? 'bg-amber-500 text-white shadow-md shadow-amber-200' : 'text-slate-600 bg-amber-50 hover:bg-amber-100',
                                emerald: isActive ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'text-slate-600 bg-emerald-50 hover:bg-emerald-100',
                                indigo:  isActive ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'text-slate-600 bg-indigo-50 hover:bg-indigo-100',
                                blue:    isActive ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-slate-600 bg-blue-50 hover:bg-blue-100',
                                rose:    isActive ? 'bg-rose-600 text-white shadow-md shadow-rose-200' : 'text-slate-600 bg-rose-50 hover:bg-rose-100',
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`
                                        flex flex-row items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold rounded-full transition-all duration-200 flex-shrink-0 whitespace-nowrap snap-start
                                        ${currentStyle}
                                    `}
                                >
                                    <tab.icon className={`w-4 h-4 ${isActive ? 'text-white' : 'opacity-70'}`} />
                ...
                {/* --- SOA TAB --- */}
                {activeTab === 'SOA' && (
                    <div className="p-4 md:p-8 bg-white min-h-[600px] flex flex-col items-center">
                        {/* Responsive Scroll Wrapper for SOA Document */}
                        <div className="w-full overflow-x-auto rounded-xl shadow-lg border border-slate-200">
                            <div className="min-w-[800px] bg-white p-10 font-mono text-sm">
                        ...
                        <div className="mt-6 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                            <button className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 font-bold active:scale-95 transition-all"><Download className="w-4 h-4"/> Download PDF</button>
                            <button className="flex items-center justify-center gap-2 px-4 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 font-bold active:scale-95 transition-all"><FileSpreadsheet className="w-4 h-4"/> Export Excel</button>
                        </div>
                    </div>
                )}
                                ...
                                <div className="md:hidden space-y-3">
                                    {computedSoa.filter(i => i.type === 'Invoice').map(inv => (
                                        <div key={inv.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                            <div className="flex justify-between items-start mb-2">
                                                <div>
                                                    <div className="font-bold text-slate-900">{inv.transNo}</div>
                                                    <div className="text-xs text-slate-500">{inv.date}</div>
                                                </div>
                                            ...
                                            <div className="flex justify-between items-end border-t border-slate-50 pt-2 mt-2">
                                                <div className="text-xs text-slate-500">
                                                    <div className="mb-0.5">Net: {(inv.debit * 0.95).toFixed(2)}</div>
                                                    <div>VAT: {(inv.debit * 0.05).toFixed(2)}</div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="text-[10px] uppercase text-slate-400 font-bold">Total Amount</div>
                                                    <div className="font-bold text-slate-900 text-lg">AED {inv.debit.toFixed(2)}</div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div>
                                ...
                                <div className="md:hidden space-y-3">
                                    {creditNotes.length > 0 ? creditNotes.map(cn => (
                                        <div key={cn.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                            <div className="flex justify-between items-start mb-2">
                                                <div>
                                                    <div className="font-bold text-slate-900">{cn.transNo}</div>
                                                    <div className="text-xs text-slate-500">{cn.date}</div>
                                                </div>
                                            ...
                                            <div className="flex justify-between items-center text-xs">
                                                <span className="font-mono text-slate-500">Ref: {cn.ref}</span>
                                                <span className="font-bold text-green-600 text-sm">AED {cn.credit.toFixed(2)}</span>
                                            </div>
                                        </div>
                                    )) : (
                                        <div className="text-center py-4 text-slate-500 text-sm">No credit notes found.</div>
                                    )}
                                </div>
    ...
    return (
        <div className="flex flex-col md:flex-row h-auto md:h-[calc(100vh-100px)] overflow-hidden bg-slate-100 rounded-xl border border-slate-200">
            ...
            <div className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-slate-200 flex-shrink-0 flex flex-col h-auto md:h-full">
                ...
                {/* Mobile Scroll / Desktop List */}
                <div className="p-3 md:p-3 flex overflow-x-auto hide-scrollbar gap-3 snap-x md:flex-col md:space-y-1 md:overflow-visible md:snap-none">
                    {reportTypes.map((item) => (
                        <button
                            key={item.id}
                            onClick={() => { setActiveReport(item.id); setSearchQuery(''); setStatusFilter('All'); }}
                            className={`flex flex-row md:flex-row items-center justify-center md:justify-start gap-2 md:gap-3 px-4 py-2 md:px-4 md:py-3 rounded-full md:rounded-lg text-sm font-bold transition-all flex-shrink-0 whitespace-nowrap snap-start ${
                                activeReport === item.id 
                                ? 'bg-slate-900 text-white shadow-md ring-1 ring-slate-700 md:bg-red-50 md:text-red-700 md:ring-red-100' 
                                : 'text-slate-600 bg-slate-100 hover:bg-slate-200 md:bg-transparent md:hover:bg-slate-50'
                            }`}
                        >
                            <item.icon className={`w-4 h-4 ${activeReport === item.id ? 'text-white md:text-red-600' : 'text-slate-500'}`} />
            ...
            case 'Users':
                return (
                    <div className="space-y-6 animate-in fade-in duration-300">
                        ...
                        <div className="md:hidden space-y-3">
                            {users.map(user => (
                                <div key={user.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center">
                                    <div>
                                        <div className="font-bold text-slate-900">{user.name}</div>
                                        <div className="text-xs text-slate-500">{user.role} • {user.email}</div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <span className={`inline-flex px-2 py-1 rounded text-xs font-medium ${user.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                            {user.status}
                                        </span>
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="w-full lg:w-64 flex-shrink-0">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden sticky top-24">
                        ...
                        <div className="flex lg:flex-col overflow-x-auto hide-scrollbar p-3 lg:p-1 gap-3 lg:gap-0 snap-x">
                            {tabs.map(tab => (
                                <button
                                    key={tab.id}
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`
                                        flex items-center justify-center lg:justify-start gap-2 lg:gap-3 px-4 py-2.5 lg:px-4 lg:py-4 text-sm font-bold transition-all rounded-full lg:rounded-none flex-shrink-0 whitespace-nowrap snap-start
                                        lg:border-b lg:last:border-b-0 lg:border-l-4 
                                        ${
                                        activeTab === tab.id 
                                        ? 'bg-slate-900 text-white shadow-md lg:shadow-none lg:bg-slate-50 lg:text-red-600 lg:border-red-600 lg:border-slate-100' 
                                        : 'bg-slate-100 text-slate-600 hover:bg-slate-200 lg:hover:bg-slate-50 lg:hover:text-slate-900 lg:border-transparent lg:bg-white lg:border-slate-100'
                                    }`}
                                >
                                    <div className={`p-1 rounded-full ${activeTab === tab.id ? 'lg:bg-red-100' : 'lg:bg-slate-100'}`}>
                                        <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-white lg:text-red-600' : 'text-slate-500'}`} />
                                    </div>
...
const AddProductModal = ({ onClose, initialData, mode = 'add', onSave, defaultType = 'Product' }) => {
    ...
    return (
        ...
        >
            <fieldset disabled={isViewOnly} className="space-y-8 pb-4 min-w-0 border-0 m-0 p-0 block">
...
const ProductManagementPage = ({ currentView, setCurrentView, products, setProducts, updateRequests, setUpdateRequests }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Tab Navigation - ENHANCED MULTI-COLORED GRID */}
            <div className="bg-white border-b border-slate-200 px-4 -mx-4 sm:-mx-6 sticky top-0 z-10 shadow-sm">
                <div className="flex overflow-x-auto hide-scrollbar gap-3 py-3 px-2 bg-white/95 backdrop-blur-sm rounded-b-xl snap-x">
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    flex items-center justify-center py-2.5 px-5 rounded-full text-sm font-bold transition-all duration-200 shadow-sm flex-shrink-0 whitespace-nowrap snap-start
                                    ${currentStyle}
                                `}
                            >
                                {tab.label}
                    ...
                    <div className="overflow-x-auto">
                        ...
                        {activeTab === 'update_req' ? (
                            <>
                                ...
                                <table className="hidden lg:table w-full text-left border-collapse table-fixed min-w-[1200px]">
                                    <thead className="bg-slate-50 border-b border-slate-200">
...
const OrderManagementPage = ({ purchaseOrders, setPurchaseOrders }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
                {/* 1. Tabs & Toolbar Section - ENHANCED MULTI-COLORED GRID */}
                <div className="border-b border-slate-200 p-3">
                    <div className="flex overflow-x-auto hide-scrollbar gap-3 snap-x">
                        {tabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`
                                        relative flex flex-row items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold rounded-full transition-all duration-200 flex-shrink-0 whitespace-nowrap shadow-sm snap-start
                                        ${currentStyle}
                                    `}
                                >
                                    ...
                                    {count > 0 && (
                                        <span className={`
                                            px-1.5 py-0.5 rounded-full text-[9px] font-extrabold min-w-[16px] text-center
                                            ${isActive ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-500'}
                                        `}>
                                            {count}
                                        </span>
                                ...
                                {selectedPO.items.map((item, idx) => (
                                    <div key={idx} className="p-4 flex flex-col gap-2">
                                        <div className="flex justify-between items-start gap-3">
                                            <div className="flex gap-3">
                                                <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 bg-slate-100 text-slate-500 text-xs font-bold rounded-full mt-0.5">
                                                    {idx + 1}
                                                </span>
                                                <div>
                                        ...
                                        <div className="grid grid-cols-3 gap-2 mt-2 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100">
                                            <div className="text-center border-r border-slate-200">
                                                <span className="block text-[10px] uppercase tracking-wider text-slate-400 mb-1">Qty</span>
                                                <span className="font-bold text-slate-700 text-sm">{item.qty}</span>
                                            </div>
                                            <div className="text-center border-r border-slate-200">
                                                <span className="block text-[10px] uppercase tracking-wider text-slate-400 mb-1">Unit Price</span>
                                                <span className="font-medium text-slate-700">AED {item.unitPrice.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                                            </div>
                                            <div className="text-center">
                                                <span className="block text-[10px] uppercase tracking-wider text-slate-400 mb-1">Total</span>
                                                <span className="font-bold text-slate-900">AED {(item.qty * item.unitPrice).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
...
const MobileBottomNav = ({ currentView, setCurrentView }) => {
    ...
    const navItems = [
        { id: 'dashboard', name: 'Dashboard', icon: LayoutDashboard },
        { id: 'orders', name: 'Orders', icon: ShoppingCart }, // Moved Orders up for better access
        { id: 'products-overview', name: 'Products', icon: Package },
        { id: 'services-overview', name: 'Services', icon: Briefcase },
        { id: 'service-rfqs', name: 'RFQs', icon: FileQuestion }, 
        { id: 'bidding', name: 'Bidding', icon: Gavel },
        { id: 'grn', name: 'GRN', icon: ClipboardCheck },
        { id: 'payments', name: 'Payments', icon: CreditCard },
        { id: 'reports', name: 'Reports', icon: BarChart3 },
        { id: 'settings', name: 'Settings', icon: Settings },
        { id: 'help', name: 'Help', icon: CircleHelp },
    ...
    const isItemActive = (item) => {
        return currentView === item.id || 
               (item.id === 'products-overview' && currentView.startsWith('products-')) || 
               (item.id === 'services-overview' && currentView.startsWith('services-'));
    };
    ...
    return (
        <>
            ...
            {showMore && (
                <>
                    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setShowMore(false)}></div>
                    <div className="fixed bottom-20 left-4 right-4 bg-white rounded-2xl shadow-2xl z-50 p-6 animate-in slide-in-from-bottom-10 fade-in duration-200">
                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
                            <h3 className="text-sm font-bold text-slate-900">All Apps</h3>
                            <button onClick={() => setShowMore(false)} className="p-1 bg-slate-100 rounded-full text-slate-500"><X className="w-4 h-4"/></button>
                        </div>
                            ...
                            {secondaryItems.map((item) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex flex-col items-center justify-start gap-2 transition-all ${
                                            isActive ? 'text-red-600' : 'text-slate-600 hover:text-slate-900'
                                        }`}
                                    >
                                        <div className={`p-3 rounded-xl shadow-sm ${isActive ? 'bg-red-50 ring-1 ring-red-100' : 'bg-slate-50 border border-slate-100'}`}>
            ...
            {/* Bottom Bar */}
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <div className="flex justify-around items-center h-16 px-2">
                    ...
                    <button
                        ...
                        className={`flex flex-col items-center justify-center w-full h-full transition-colors ${
                            showMore || secondaryItems.some(isItemActive) ? 'text-red-600' : 'text-slate-400 hover:text-slate-600'
                        }`}
                    >
                        <div className={`flex items-center justify-center w-6 h-6 mb-1 rounded-md transition-all ${showMore ? 'bg-red-50' : ''}`}>
...
const Header = ({ onLogout, currentView }) => {
  ...
  if (userState === 'approved') {
    ...
    return (
      <div className="flex h-screen w-full bg-slate-100 font-sans antialiased text-slate-600 text-sm" dir={language === 'ar' ? 'rtl' : 'ltr'}>
        {/* Sidebar Removed for Desktop as well */}
        
        <main className="flex-1 flex flex-col overflow-y-auto pb-24">
          <Header onLogout={handleLogout} currentView={currentView} />
